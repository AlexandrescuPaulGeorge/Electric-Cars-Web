...
while True:
	...
	if numeResursaCeruta == '/':
		numeResursaCeruta = 'continut/index.html'
	...
	try:
		...
		clientsocket.sendall('Content-Length: {0} \r\n'.format(str(os.stat(numeFisier).st_size)).encode());
		clientsocket.sendall('Content-Type:  {0} \r\n'.format(tipMedia).encode());
